...
import { formatDateUTC, formatNumber, formatRelative } from '@/lib/utils';
    ...
    >
      <div className="min-w-0">
        ...
        <div className="mt-1.5 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          ...
          <span>
            <span className="mono tabular-nums text-foreground">
              {entry.summary.analysisCount}
            </span>{' '}
            analyzed
          </span>
          {totalBugs > 0 && (
            <span>
              <span className="mono tabular-nums text-foreground">{totalBugs}</span>{' '}
              bug{totalBugs === 1 ? '' : 's'}
            </span>
          )}
        </div>
